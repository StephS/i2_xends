...
module rod_clamp(smooth_rod=x_axis_smooth_rod_diameter){
	difference(){
		...
		// smooth rod hole
		translate([0,0,-0.01]) rod_hole(d=smooth_rod,h=frame_threaded_smooth_rod_clamp_height+0.02);
		// frame screw hole
		translate([v_rod_hole(d=smooth_rod)/2+frame_threaded_to_smooth_rod_gap +screw_dia(v_screw_hole(type=frame_threaded_rod, $fn=12, horizontal=true))/2, v_rod_hole(d=smooth_rod)/2+frame_threaded_to_smooth_rod_clamp_wall_thickness, frame_threaded_smooth_rod_clamp_height/2]) rotate([90,0,0]) screw_hole(type=frame_threaded_rod, h=v_rod_hole(d=smooth_rod)+frame_threaded_to_smooth_rod_clamp_wall_thickness*2, $fn=12, horizontal=true);
