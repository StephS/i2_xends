...
idler_short_side=wade_block_depth-1;
...
module wade (hotend_mount=0,legacy_mount=false){
	difference (){
		union(){
			...
			translate([10,motor_mount_translation[1]-hole_for_608/2-6,0])
			cube([wade_block_width+extra_gear_separation+2,
				wade_block_height-motor_mount_translation[1]+hole_for_608/2+6,
			...
			difference() {
				translate(idler_fulcrum){
					rotate(-15)
						translate([-(idler_hinge_r+3)+1,-idler_hinge_r-3,-wade_block_depth/2])
							translate([-1,-1,0])
					...
					rotate(-15)
						...
						translate([-1,-1,0])
						cube([idler_hinge_r+4+15,
							idler_hinge_r*2+6,
							layer_thickness]);
				}
				translate([-1, idler_fulcrum[1]-(idler_hinge_r*2+15)/2, -0.1])
					
						cube ([10, idler_hinge_r*2+15, 
						wade_block_depth/2-
						idler_short_side/2+
						idler_hinge_width+0.25-
						layer_thickness+0.1]);
				
				translate([idler_fulcrum[0],idler_fulcrum[1],-((idler_short_side-2*idler_hinge_width-0.5)-
						wade_block_depth/2-
						idler_short_side/2+
						idler_hinge_width+
						layer_thickness)-(idler_short_side-2*idler_hinge_width-0.5)]){
					translate([idler_hinge_r,0,(idler_short_side-2*idler_hinge_width-0.5)/2])
					cube([idler_hinge_r*2,idler_hinge_r*2+1,idler_short_side-2*idler_hinge_width-0.5],
						center=true);
					rotate(-30){
						cylinder(r=idler_hinge_r+0.5,
							h=idler_short_side-2*idler_hinge_width-0.5,
							center=false,$fn=60);
						translate([idler_hinge_r,0,(idler_short_side-2*idler_hinge_width-0.5)/2])
						cube([idler_hinge_r*2,idler_hinge_r*2+1,
							idler_short_side-2*idler_hinge_width-0.5],
							center=true);
					}
				}
...
//				cylinder(r=m4_nut_diameter/2,h=base_thickness,$fn=6);
				translate([0,0,base_thickness/2]) rotate([0,0,0]) nut_trap(m4_wrench,base_thickness,0);}
			}
...
module b608(h=8, support=false){
	difference() {
		translate([0,0,h/2]) cylinder(r=hole_for_608/2,h=h,center=true,$fn=60);
		if (support) translate([0,0,h/2-0.01]) cylinder(r=m8_clearance_hole/2-0.5,h=h+0.02,center=true,$fn=60);
	}
}
